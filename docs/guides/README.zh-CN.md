...
| 变量 | 适用 | 用途 |
|------|------|------|
| `PISTACK_SKILLS_DIR` | runtime（扩展） | 不依赖 symlink，从此路径读 skill |
| `PISTACK_SOURCE_SKILLS` | sync | 一次性 sync 源路径 |
| `PISTACK_PLUGINS_REPO` | sync | 覆盖 Git 仓库 |
| `PISTACK_PLUGINS_REF` | sync | branch/tag |
| `PISTACK_SKIP_FETCH` | sync | 跳过 GitHub fetch |
| `PISTACK_GIT_REF` | bootstrap | pi-stack 安装 ref |
...
Principle skills：未被全局 skill 覆盖时使用 `/skill:principle-<name>`。完整 upstream 用法：[cursor/plugins/pstack](https://github.com/cursor/plugins/tree/main/pstack)。
